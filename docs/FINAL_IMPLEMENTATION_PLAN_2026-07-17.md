...
| R-045 | 2026-07-19 | **B0 anchor COMPLETE — PASS** | Phase 2 | B0-RUN-006 (cont. of 004/005), BGE-M3+LoRA r16 CRR HN-K7, DBP5L-Ind v2 fold0_seed13, seed 42, 30 epochs, `negpol-v2-train-only`, ckpt `c99a7b6c` sel epoch 27 | Natural within-lang combined MRR 24.08 (tail 34.9 / head 13.29); masked 6.74; missing 3.88. Mention split: tail mentioned 39.83 vs unmentioned 5.22; head 41.66 vs 1.78 | none | **Sufficiency PASS:** all 12 concept-clustered tests lower 95% bound > margin 0 (min +0.79) vs both R-042 lexical experts, on unmentioned/both-dir (natural) + both-dir (masked/missing). Beats lexical floor beyond mention shortcut → remaining fallback matrix authorized. Natural aggregate NOT cited as relational generalization (unmentioned bucket is the indicator) | `eval/B0-FOLD0-SEED42/` (WSL), `lexical_comparison.json` |
| R-046 | 2026-07-19 | **Incident resolved (HN OOM)** | Phase 2 | Root cause of B0-RUN-002/003 failures, resolved via B0-RUN-004→006 | HN cache OOM fixed (`no_grad`, R-F004); 21+ clean refreshes then full 30-epoch completion | Resolves R-F003/R-F004 root cause. **Failed runs stay failed** (append-only): B0-RUN-002 (R-F003, misdiag env), B0-RUN-003 (R-F004, real bug), B0-RUN-004/005 (external GPU OOM from operator's other LLM) — all `failed`; incident closed by successful B0-RUN-006 | Linked: 002→003→004→005→006 | `lexical_runs_registry.json` |
| R-047 | 2026-07-19 | **Correction to R-045 (label only)** | Phase 2 | Per-language natural breakdown, B0 fold0_seed13, ckpt `c99a7b6c`, regenerated from `ranks_{natural,masked,missing}.json` under ckpt-SHA + view-hash (`d1afcf20`) assertions | **Natural mention split (authoritative):** combined mentioned 44.82 / unmentioned 10.89; tail 44.33 / 18.84; head 46.86 / 7.42. Per-lang unmentioned (leak-reduced indicator): EN 9.17 [6.46,12.32], FR 14.20 [8.43,23.46], ES 12.81 [8.46,18.24], JA 3.57 [2.01,5.98], EL 9.84 [6.36,15.85] | **R-045 prose defect:** its "Mention split: tail 39.83 / head 41.66" quoted the **missing**-view numbers mislabeled as natural. R-045's numeric result and its 12-test PASS are otherwise correct — the verdict read natural ranks (view hash `d1afcf20`, model unmentioned tail 18.84/head 7.42 feeding the +17.5/+3.75 lower bounds). R-045 preserved; this row corrects the label. EN point-estimate fixed at source (generator, not hand-edit) | Data card artifact + `gen_datacard.py`, `eval/B0-FOLD0-SEED42/perlang_tables.md` |
| R-048 | 2026-07-20 | **fold1 COMPLETE — PASS** | Phase 2 | B0-RUN-012 (cont. chain 007-012), fold1_seed42, seed 42, 30 epochs, ckpt `d89890da` sel epoch 28 | Natural within-lang combined MRR 23.69 (tail 34.68 / head 12.71); masked 8.15; missing 3.44. Selected acc@1 30.08% | Fold1 needed 6 continuation runs: operator manual stops (007/008) + OpenAI Codex/ChatGPT.exe desktop GPU collisions causing repeated `cudaErrorUnknown` at epoch 28 (R-F005) — resolved by closing the ChatGPT app. Full-30 run raised selection from epoch 3 (29.94) to epoch 28 (30.08), so completion mattered | **PASS:** all 12 concept-clustered tests lower 95% bound > 0 (min +0.75) vs both R-042 lexical experts. Cross-fold generality holds (fold0 +0.79, fold1 +0.75, near-identical natural MRR 24.08/23.69) | `eval/B0-FOLD1-SEED42/` |
| R-049 | 2026-07-21 | **fold2 COMPLETE — INCONCLUSIVE** | Phase 2 | B0-RUN-015 (auto-resume + manual-resume chain 013-015), fold2_seed79, seed 42, 30 epochs, ckpt `e7c506a1` sel epoch 6 | Natural within-lang combined MRR 24.69 (tail 39.03 / head 10.36); masked 6.67; missing 4.04. Natural-unmentioned tail 20.35 / head 5.44. Selected acc@1 29.49% (epoch 6; ep30 was 28.00, early best held) | fold2 needed resumes: WSL2 CUDA transient at ep8 (auto-resume wrapper) + genuine SIGILL (`Illegal instruction`, exit 132) at ep28 that the wrapper's no-advance guard correctly refused to loop on (R-F006 records the paired eval-clobber). Single manual resume with `setsid` (WSL distro was tearing down nohup jobs on launcher exit) cleared ep28 and finished | **INCONCLUSIVE:** 11/12 concept-clustered lower bounds > margin 0; **one straddles** — `masked\|name_match\|all_head` lo −0.042 (hi +0.402), macro lo −1.47, not significant. Benign: hardest stress cell incl. mentioned queries where name-matching wins by construction; its leakage-reduced counterpart `masked\|name_match\|unmentioned_head` passes (lo +0.687), and **every natural + every unmentioned cell clears zero** (natural-unmentioned min lo +2.86). Cross-fold generality holds at the headline leakage-reduced level (fold0 +0.79, fold1 +0.75, fold2 natural-unmentioned lo +2.86–19.69); the lone straddle is a mentioned-query/stress-view artifact, not a generalization failure | `eval/B0-FOLD2-SEED42/lexical_comparison.json` |
| R-F006 | 2026-07-21 | **Bug found + fixed (tooling, no result invalidated)** | Phase 2 | fold2 three-view eval was launched without `--out-dir`; `eval_three_views.py` + `compare_vs_lexical.py` both **defaulted `--out-dir`/`--fold` to fold0** (`B0-FOLD0-SEED42`/`fold0_seed13`), a clobber-prone hardcoded default | fold2's `ranks_*`, `view_*`, `completion_index`, `evaluated_checkpoint` overwrote fold0's in `eval/B0-FOLD0-SEED42/` (WSL tree not git-tracked, no VCS recovery). fold0's **verdict artifacts survived** (`lexical_comparison.json`, `perlang_tables.md`, Jul-19); only fold0's raw rank dumps were lost (regenerable; fold0 PASS already locked in R-045/R-047) | Detected immediately (output path read `B0-FOLD0-SEED42` for a fold2 run) | **Fix:** relocated fold2 outputs to `eval/B0-FOLD2-SEED42/` and repointed the moved `view_*.json` absolute `rank_dump` paths; made `--fold`/`--out-dir` **required** on both scripts and added a **cross-fold clobber guard** (refuse if the out-dir's `evaluated_checkpoint.json` fold ≠ requested fold). Both guards + selftests verified. R-049's numbers are unaffected (evaluated on fold2 ckpt+targets, confirmed via `evaluated_checkpoint.json fold=fold2_seed79`) | `scripts/baselines/eval_three_views.py`, `scripts/baselines/compare_vs_lexical.py` |
| R-050 | 2026-07-22 | **E1 screen: InfoNCE vs CRR — DROP InfoNCE** | Phase 2 | E1-INFONCE-fold0, BGE-M3+LoRA r16, HN-K7, **InfoNCE (`--use-crr 0`)**, fold0_seed13, seed 42, 30 epochs — identical to B0 except the loss. ckpt `81fc2574` sel epoch 18 | Natural within-lang combined MRR **19.04** (tail 26.82 / head 11.26); masked 5.19; missing 3.17; **natural-unmentioned 8.83**. Selected acc@1 28.02% | RDP-induced `cudaErrorUnknown` abort at ep23 (external, guard auto-resume was falsely blocked by WSL-vs-Windows autocrlf false-dirty — fixed via `git -c core.autocrlf=true`); manual `setsid` resume ep23→30. Aggregate 15.06h GPU / 2 segments | **DROP InfoNCE, retain CRR.** Head-to-head vs B0 CRR (R-045): CRR wins **every** view/direction — natural **+5.04** (24.08 vs 19.04), natural-unmentioned **+2.06** (10.89 vs 8.83), tail +8.05, head +2.03, masked +1.55, missing +0.71. Gap ≫ either model's clustered CI half-width → outside noise. **Selection acc@1 was a wash (28.02 vs 27.72) yet did NOT predict test MRR** — InfoNCE generalizes ~21% worse; confirms the rule that val acc@1 is not the outcome. InfoNCE still clears the lexical floor (sufficiency PASS, min clustered lo +0.705), so it is a valid-but-weaker competitor. CRR earns its place on v2. Single-seed screening (mechanism); no multi-seed needed to drop | `eval/E1-INFONCE-fold0/` |
...
- [x] Lexical BM25/name-match baseline. *(R-042 lexical experts on all 3 folds; retained as the sufficiency floor. `name_match` = mention/leakage detector only, BM25 = genuine lexical expert. Every B0/E-run verdict is measured against both via concept-clustered bootstrap.)*
- [x] Degree/PPR structural heuristics. *(R-033/R-034: PPR MRR 0.05–0.06 ≈ degree 0.06–0.076 ≫ random 0.001, ≪ text 0.27 — structural signal is popularity-like and does not explain text performance. Answer edges excluded from traversal.)*
- [ ] mBERT bi-encoder under the exact BGE training/evaluation recipe. *(Planned — Phase-2 step 15, 5 final runs.)*
- [ ] XLM-R bi-encoder under the exact recipe. *(Planned — Phase-2 step 16, 5 final runs.)*
- [ ] Clean BGE-M3/LoRA B0 with three seeds. *(**Partial — cross-fold done, seeds pending.** B0 complete on all 3 concept-disjoint folds @ seed 42: fold0 PASS (R-045, natural MRR 24.08), fold1 PASS (R-048, 23.69), fold2 INCONCLUSIVE (R-049, 24.69 — 1/12 stress cell straddles). Cross-fold = **2 PASS + 1 INCONCLUSIVE**; headline natural/unmentioned generality holds. Seeds 123/777 on fold0 (2 runs) deferred to final confirmation per the revised fold/seed rule.)*
- [ ] One corrected structural inductive baseline on the v2 folds. *(Planned — Phase-2 step 17, 5 final runs.)*
...
- [x] InfoNCE with in-batch negatives. *(E1, R-050 — Phase-2 step 2. **DROPPED.** InfoNCE (`--use-crr 0`) natural test MRR 19.04 (unmentioned 8.83) vs B0 CRR 24.08 (10.89); CRR wins every view/direction (natural +5.04). Selection acc@1 was a wash (28.02 vs 27.72) yet did NOT predict test MRR — InfoNCE generalizes ~21% worse. Still clears the lexical floor (sufficiency PASS).)*
- [ ] CRR with identical negatives. *(**Running — E3, Phase-2 step 3.** CRR without HN7 (`--hard-neg-k 0`, in-batch CRR = identical negatives to InfoNCE), else identical to B0, fold0/seed42. Isolates whether HN7 helps on v2. ckpt `…_0933_crr_r16`; verdict pending vs B0 (HN7) and E1 (InfoNCE).)*
- [x] CRR plus seven fresh hard negatives. *(= B0 anchor, R-045/R-048/R-049 — Phase-2 step 1. CRR + HN-K7 from epoch 5, hard negatives from fold-train entities only. This is the retained baseline: beats InfoNCE and the lexical floor across folds.)*
- [ ] Dynamic top-ranked negatives refreshed on a fixed schedule. *(Planned — Phase-2 step 7, false-negative-safe dynamic HN.)*
- [ ] Self-adversarial weighting as an optional cheap control. *(Planned — optional control.)*
