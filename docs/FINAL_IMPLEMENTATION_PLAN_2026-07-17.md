...
- [~] Personalized PageRank and shortest-path shortcut baselines. *(`audit_v2_ppr_shortcut.py`: traversal graph excludes ALL valid/test answer edges (target-removed by construction), relation types dropped; PPR + BFS on a 400-target/fold sample. Running — result → R-033.)*
