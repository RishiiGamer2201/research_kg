...
def main() -> None:
    if len(sys.argv) != 2 or sys.argv[1] not in {"1", "2", "3", "4"}:
        print("Usage: python scripts/start_wn18rr_split_detached.py <1|2|3|4>", file=sys.stderr)
        raise SystemExit(2)

    split = sys.argv[1]
    script = ROOT / "scripts" / "run_wn18rr_split_gpu.sh"
    log = LOG_DIR / f"wn18rr_v{split}_detached.log"
