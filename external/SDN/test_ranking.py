...
def get_rank(neg_links):
    ...
    if head_target_id != 10000:
        ...
        head_scores = model_(data)
        if isinstance(head_scores, tuple):
            head_scores = head_scores[0]
        head_scores = head_scores.squeeze(1).detach().numpy()
    ...
    if tail_target_id != 10000:
        ...
        tail_scores = model_(data)
        if isinstance(tail_scores, tuple):
            tail_scores = tail_scores[0]
        tail_scores = tail_scores.squeeze(1).detach().numpy()
