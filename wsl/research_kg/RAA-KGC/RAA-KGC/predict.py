...
from doc import collate, Example, Dataset, collate_test
...
class BertPredictor:
    ...
    def predict_by_examples(self, examples: List[Example]):
        data_loader = torch.utils.data.DataLoader(
            Dataset(path='', examples=examples, teset_set=False),
            num_workers=1,
            batch_size=args.batch_size,
            collate_fn=collate,
            shuffle=False)

        hr_tensor_list, tail_tensor_list, related_hr_tensor_list = [], [], []
        for idx, batch_dict in enumerate(data_loader):
            ...
            related_hr_tensor_list.append(outputs['related_hr_vector'])

        return torch.cat(hr_tensor_list, dim=0), torch.cat(tail_tensor_list, dim=0), torch.cat(related_hr_tensor_list, dim=0)
    ...
    def predict_by_entities(self, entity_exs) -> torch.tensor:
        ...
        data_loader = torch.utils.data.DataLoader(
            Dataset(path='', examples=examples, teset_set=True),
            num_workers=2,
            batch_size=args.batch_size,
