...
from triplet import EntityDict, EntityExample
...
# Iterate over all relations and head-relation pairs in the triplet dictionary
def find_all_heads_for_tail(entity_id: str, cur_ex_tail_id, all_triplet_dict, mask_indices):
    '''
    entity_id:         current head entity id
    cur_ex_tail_id:    current tail entity id
    all_triplet_dict:  all triples dictionary
    '''
    for (head_id, relation), tail_ids in all_triplet_dict.hr2tails.items():
        if entity_id in tail_ids:
            if head_id == cur_ex_tail_id:
                continue
...
@torch.no_grad()
def compute_metrics(related_hr_tensor: torch.tensor,
                    hr_tensor: torch.tensor,
                    entities_tensor: torch.tensor,
                    target: List[int],
                    examples: List[Example],
                    k=20, batch_size=256) -> Tuple:
    '''
    related_hr_tensor: the embedding (head-anchor, relation, tail-anchors)
    hr_tensor:         the embedding (head-anchor, relation)
    entities_tensor:   the embedding of tail entities set t
    target:            target entity
    examples:          the all triples
    k:                 save top-k prediction and its probability
    '''
    assert hr_tensor.size(1) == entities_tensor.size(1)
    assert hr_tensor.size(0) == related_hr_tensor.size(0)
    ...
    for start in tqdm.tqdm(range(0, total, batch_size)):
        ...
        # Calculate scores within batchsize using matrix multiplication
        # hr_tensor is a two-dimensional tensor
        # entities_tensor is a two-dimensional tensor
        ...
        rerank_by_graph(related_batch_score, batch_score, examples[start:end], entity_dict=entity_dict)
        ...
        for idx in range(batch_score.size(0)):
            ...
            mask_indices = find_all_heads_for_tail(cur_ex.head_id, cur_ex.tail_id, all_triplet_dict, mask_indices)
...
def predict_by_split():
    ...
    backward_metrics = eval_single_direction(predictor,
                                             entity_tensor=entity_tensor,
                                             eval_forward=False)
    # the avarage of head entity prediction and tail entity prediction
    ...
    with open('{}/task_hrt_{}_{}.json'.format(prefix, split, basename), 'w', encoding='utf-8') as writer:
...
def eval_single_direction(predictor: BertPredictor,
    ...
    # embedding (h,r) and (h,r,tail-anchors)
    ...
    # compute metrics
    topk_scores, topk_indices, metrics, ranks = compute_metrics(related_hr_tensor=related_hr_tensor,
                                                                hr_tensor=hr_tensor,
    ...
    prefix, basename = os.path.dirname(args.eval_model_path), os.path.basename(args.eval_model_path)
    split = os.path.basename(args.valid_path)
    with open('{}/task_hrt_{}_{}_{}.json'.format(prefix, split, eval_dir, basename), 'w', encoding='utf-8') as writer:
