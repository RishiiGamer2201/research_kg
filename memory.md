...
- **P1.6 Part B DONE (ledger R-030) — code only:** reciprocal head+tail in BOTH eval + train.
  - `eval_dbp5l.py --directions tail,head`: rev_filter_map (r,t)→heads, `reverse of` marker, reports head/tail/combined SEPARATELY (+ `by_direction` in save + manifest). Validated: tail wl-MRR **1.44016 == R-020** (backward compat), head/combined populate. Default `tail` unchanged.
  - `train_dbp5l_lora.py --reciprocal 1`: adds head example per triple (tail [SEP] `reverse of` rel → head), shared `RECIP_MARKER`, distinct `_recip` cache tag. Smoke: 236,512=2×118,256 examples, trains+saves exit 0. Default 0 = tail-only.
  - **KEY SCOPE FACT:** trainer+evaluator read OLD `processed/{train,valid,test}.json`, NOT the v2 folds. So real head/tail/combined numbers + v2 benchmark eval need **v2-fold wiring = Phase 2 (P2.1)**. Did NOT launch a 16h old-split reciprocal retrain (would be mechanism-only, not headline). Smoke checkpoints cleaned up.
- **PHASE 1 COMPLETE — Gate G1 CLOSED (2026-07-18).** DBP5L-Ind v2 built + audited + documented.
  - Concepts (29,440) → 3 folds [13,42,79] concept-disjoint → budgets 0/1/3/5 (comparability rule + inverse-clean ordered) → descriptions (native/cross-lang/name, no LLM) → tracks (primary/oracle/masked/missing-text/train-graphtext) → audits → data card `docs/DBP5L_IND_V2_DATACARD.md`.
  - Reciprocal head+tail in train+eval (R-030); v2 eval wiring `--v2-targets` deterministic (R-037). **Finding: unmentioned MRR ~1% ≪ mentioned 5-18% → natural-text MRR driven by answer mentions, not reasoning.** PPR 0.05≈degree≪text (no shortcut explaining text). v1 concept-leak 72.3% → v2 disjoint.
  - Ledger R-024–R-037. All builders `--selftest`. Pushed to main.
  - **Phase 2 carry:** wire v2 folds into TRAINING (trainer+eval still read old processed/*.json); 3-seed clean B0 + trained head/tail/combined on v2 = real headline; clean-vs-contaminated 3-seed. Phase 4: corruption track. P2.6: S2DN target-edge removal.
- **NEXT: Phase 2 (P2.1)** — clean baselines on v2 folds: wire folds into train/eval; lexical/PPR/mBERT/XLM-R/BGE-M3-LoRA B0 (3 seeds); then screen extensions.
- **Phase 2 TODO (carried):** wire v2 folds/budgets/tracks into train+eval (fold concept→triple filtering, budget support exposure, track candidate universes, primary vs oracle desc view); then 3-seed clean B0 + reciprocal head/tail/combined on v2 = real headline. Clean-vs-contaminated 3-seed comparison also here.
...
### 2026-07-18 — Phase 2 in flight (B0-RUN-002) + REPORT TEMPLATE (user-specified)
- **B0-RUN-002**: pid 19926, fold0_seed13/seed42, launch commit d51630b, ~14.75 GPU-h, started 08:40:41Z. Health: `bash scripts/baselines/check_run.sh DBP5L/ind_v2/audits/training/B0-RUN-002` (NEVER pgrep).
- **On completion, run unattended (already built + self-checked):**
  1. `scripts/baselines/eval_three_views.py --train-run-dir <ckpt dir> --out-dir DBP5L/ind_v2/audits/eval/B0-FOLD0-SEED42` (verifies ckpt SHA vs selection_record.json; natural/masked/missing; head+tail+combined; rank dumps; atomic+resumable)
  2. `scripts/baselines/compare_vs_lexical.py --eval-dir <that dir> --fold fold0_seed13` (exit 0 only on PASS)
- **REPORT ORDER (user requirement):** (a) checkpoint + selection record FIRST (ckpt SHA-256, selection metric/value/epoch, runtime profile); (b) THEN evaluation outcomes; (c) **clustered query-weighted ratio bootstrap = PRIMARY**, **concept-macro = robustness only**, per-query bootstrap = diagnostic only; (d) continuation verdict PASS/FAIL/INCONCLUSIVE.
- **Verdict rules:** PASS needs clustered lower 95% bound > margin for BOTH unmentioned directions (natural) AND both directions (masked+missing), vs EVERY lexical method. CI straddling margin = INCONCLUSIVE. Upper bound < margin = FAIL. Missing/misaligned dumps = INCONCLUSIVE, never PASS. Report n_unique_concepts + cluster-size distribution beside every CI.
