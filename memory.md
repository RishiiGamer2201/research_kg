...
- **NEXT: P0.3** deterministic eval: max_length from ckpt (already auto-read — verify), persist candidate IDs+order, add train/valid/test/support to filtered-known map, verify head+tail templates, average tied ranks, assert identical metrics on repeat. Then P0.4 relation identity, P0.5 reproduce baseline, Gate G0.
