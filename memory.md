...
- **NEXT: P1.4** nested support budgets 0/1/3/5 per unseen entity (deterministic ordered pool, prefer train-connected non-target non-dup edges, materialize k∈{0,1,3,5} after global dedup, remove support edges from eval targets + add to filter). Then P1.5 train-only descriptions, P1.6 eval tracks (+ deferred head-prediction), rest P1.7 (dup/inverse/answer-string/PPR), P1.8 data card → Gate G1 (push).
