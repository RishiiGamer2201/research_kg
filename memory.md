...
- **NEXT: rest of P1.7 audits** (per user: exact+semantic answer leakage on the preserved snapshot text; plus dup/near-dup entity, inverse-relation/reciprocal-triple, answer-string/relation-template, degree/relation-freq/component-size/language distributions, PPR + shortest-path shortcut baselines). Then **P1.8 data card** → **Gate G1** (push all Phase-1 commits).
