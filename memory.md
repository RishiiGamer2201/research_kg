...
- **NEXT: P1.6** eval tracks: primary alignment-free full-candidate head+tail; oracle-alignment diagnostic; within-lang + all-lang candidates; budgets 0/1/3/5; clean/missing-text/corruption tracks. **Includes deferred head-prediction eval (reciprocal query + direction marker + reciprocal training).** Then rest P1.7 (exact+semantic answer leakage [user], dup/inverse/answer-string/degree/PPR shortcut), P1.8 data card → Gate G1 (push).
