...
- **NEXT: P1.5** rebuild descriptions from train-only evidence (source priority: native verified snapshot → verified cross-lang → native name; NO LLM text in clean track; train entities may use graph-derived text from TRAIN triples only; valid/test exclude answer edges + post-cutoff info; record source/lang/time/hash/fallback per entity). Then P1.6 eval tracks (+deferred head-prediction), rest P1.7 (dup/inverse/answer-string/PPR/degree/PPR-shortcut), P1.8 data card → Gate G1 (push).
