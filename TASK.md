> WARNING **EVAL BUG FIXED (2026-07-03):** All evals before this date used max_length=96 at eval time regardless
> of training max_length. Run B was trained at ml=128 but evaluated at ml=96 → underestimated.
> Corrected: Run B = **24.76% MRR** (was 15.59%), Run A = **23.41%** (was 14.95%).
> eval_dbp5l.py now auto-detects max_length from checkpoint. All prior evals should be re-run.
...
**Deadline (CONFIRMED, official CFP, UTC-12):** Abstract **Jul 21** | Full Paper **Jul 28** | Supplementary+code **Jul 31**
**Format:** 7 pages main content + refs-only pages 8-9 (AuthorKit27) | OpenReview; **register profile now**

---

## WARNING PUBLISHABILITY ASSESSMENT (added 2026-07-02)
...
## [P0] PHASE A; Must-Do Before Writing Continues (NEW, do this first, ~5-6 days)
...
## CURRENT STATUS (updated 2026-07-03; see docs/IMPLEMENTATION_PLAN.md + docs/RESULTS_AND_INFERENCE.md)
- **Best model:** **Run E**; CRR + HN K=7 + max_length **160** + LoRA rank 16 → **MRR 26.69%**
  (filtered, final). NOTE: the old 15.59% and "Run B / ml=128 best" below are SUPERSEDED; two
  eval bugs (hardcoded eval max_length=96; support triples missing from filter map) were fixed;
  correcting them lifted the headline to 26.69% and flipped the winner to ml=160 (Run E > Run B).
- **GPU:** 3-seed chain of Run E config (ml=160) running (user-launched) → mean±std + significance.
- **Next:** W2 anchor-aware eval (resolve XL negative result / Gate G1) · SimKGC-mBERT baseline ·
  JA error analysis · Related Work. Framing LOCKED = benchmark-first.
...
## Phase 1; CRR Loss Implementation [done]
...
## Phase 2; Architecture & Data Preparation [done]

### Model (train_dbp5l_lora.py) [done]
...
### DBP-5L-Ind Benchmark [done]; PAPER CONTRIBUTION #1
...
### Entity Description Enrichment [done]
...
## Phase 3; Core Training Runs [done]
...
## Phase 3b; Hard Negative Mining [done]
...
## Phase 3c; Performance Push (Enriched Descriptions) [done]

| Run | Config | MRR | H@1 | H@10 | vs 3-HN |
|-----|--------|-----|-----|------|---------|
| **A** | CRR + HN K=7 + ml=128 + enriched desc | **23.41%** | 15.28% | 39.00% | (corrected ml=128) |
| **B** | + LoRA rank 16 | **15.59%** | 9.78% | 26.57% | +37% ← **BEST** |
| C | + Global Neg (stale cache) | 13.20% | 6.93% | 25.06% | -15% [fail] |
...
|-----|--------|--------|-----|------|
| E | CRR + HN K=7 + **ml=160** + LoRA r16 | [done] Done | **14.89%** | Worse than B; ml=128 is optimal |
| **F** | CRR + HN K=7 + ml=128 + LoRA r16 + **40ep lr=3e-4** | [running] **RUNNING** | TBD | Run B + more convergence |
| G | CRR + HN **K=15** + ml=128 + LoRA r16 | [paused] **DEPRIORITIZED** | TBD | Behind Phase A now; only run if A1-A5 land early |
...
| 4   | + max_length 128 + rich descriptions (corrected) | 23.41% | 15.28% | 26.38% | 39.00% |
| **5** | **+ LoRA rank 16 (Full Model, corrected)** | **24.76%** | **16.74%** | **27.89%** | **39.94%** |
...
> WARNING All rows above the "(new)" additions are internal-only comparisons. Table 1 is NOT reviewer-ready
...
> WARNING Revised guidance (2026-07-02): numbers from Run B are stable enough to draft FROM, but do not
...
| Date | Milestone |
|------|-----------|
| Jul 2 | Run F training + Start Phase A planning |
| Jul 2-4 | A4 framing + A1 ALIGNKGC setup + A2 seed runs queued |
| Jul 4-7 | A1 ALIGNKGC results + A5 Related Work draft |
| Jul 7-10 | Sections 5, 4, 3 draft + A3 significance tests |
| Jul 10-14 | Sections 1, 6 + B3 JA error analysis |
| Jul 14-18 | Full draft review + LaTeX formatting |
| Jul 18-20 | Final polish -> **Abstract submitted Jul 20** |
| Jul 20-27 | Reviewer-ready final paper -> **Submitted Jul 27** |
